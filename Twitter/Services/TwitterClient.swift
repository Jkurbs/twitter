...
protocol TwitterClientType {
    ...
    func logIn(username: String, password: String, _ completion: (Result<Bool, Swift.Error>) -> Void)
    func logOut()
...
final class TwitterClient: TwitterClientType {
    ...
    private init() {
        ...
        isLoggedIn.observeNext {
            UserDefaults.standard.set($0, forKey: TwitterClient.isLoggedInKey)
        }
        .dispose(in: bag)
    }
}
...
extension TwitterClient {
    ...
    func logIn(username: String, password: String, _ completion: (Result<Bool, Swift.Error>) -> Void) {
        ...
        } else if users[username] == nil {
            completion(.failure(Error.invalidUsername))
        } else {
            completion(.failure(Error.invalidPassword))
        }
    }
